...
final class HousekeepingListCommandTest extends TestCase
{
    ...
    public function testListJsonIsPrettyPrintedAndKeepsSlashesUnescaped(): void
    {
        $dir = sys_get_temp_dir() . '/agent-cron-list-json-format-' . bin2hex(random_bytes(4));
        $configFile = $dir . '/tasks.php';
        (new Filesystem())->mkdir($dir);
        file_put_contents($configFile, '<?php return ' . var_export([
            'tasks' => [
        ...
        try {
            ...
            self::assertStringContainsString('"name": "custom/slash-task"', $display);
            self::assertStringContainsString('"provider": "provider/with-slash"', $display);
