...
final class HousekeepingNextCommandTest extends TestCase
{
    ...
    public function testNextCommandFormatsTableTimestampsAndNextRunInUtc(): void
    {
        $dir = sys_get_temp_dir() . '/agent-cron-next-formatting-' . bin2hex(random_bytes(4));
        $configFile = $dir . '/tasks.php';
        $stateFile = $dir . '/state/state.json';
        $lastFinishedAt = 1234567890;
        $intervalSeconds = 2_000_000_000;
        $nextDueAt = $lastFinishedAt + $intervalSeconds;
        (new Filesystem())->mkdir(dirname($stateFile));
        file_put_contents($stateFile, json_encode([
            'schema_version' => 1,
            'tasks' => [
    ...
    public function testNextCommandFormatsMissingTimestampAndDueTaskAsNow(): void
    {
        $dir = sys_get_temp_dir() . '/agent-cron-next-due-' . bin2hex(random_bytes(4));
        $configFile = $dir . '/tasks.php';
        $stateFile = $dir . '/state/state.json';
        (new Filesystem())->mkdir(dirname($stateFile));
        file_put_contents($stateFile, json_encode([
